...
function WomansFashion() {
  const dispatch = useDispatch();
  const products = useSelector((state) => state.product.product);
  const [currentPage, setCurrentPage] = useState(1);
  const productsPerPage = 6;

  const womansFashionCategories = [
    "beauty",
    "skin-care",
    "tops",
    "womens-bags",
  ...
  const womansFashionClothes = products.filter((item) =>
    womansFashionCategories.includes(item.category)
  );

  const totalPages = Math.ceil(womansFashionClothes.length / productsPerPage);
  const indexOfLastProduct = currentPage * productsPerPage;
  const indexOfFirstProduct = indexOfLastProduct - productsPerPage;
  const paginatedProducts = womansFashionClothes.slice(indexOfFirstProduct, indexOfLastProduct);
